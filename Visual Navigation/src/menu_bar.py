...
class menuBar(Ui_MainWindow):
	...
	def savePath(self):
	    ...
	    for line in self.clicksPositions:
	        realRobotCoordenates.append([round(200/(winWidth/line[0])), round(130/(winHeight/(winHeight-line[1])))])

	    path = QtWidgets.QFileDialog.getSaveFileName(self.centralwidget, "Guardar Como", os.getcwd(),"Mbarete files (*.mbarete)")
	    print(path)
	    try:
	...
	def openExistingPath(self):
	    ...
	    path = QtWidgets.QFileDialog.getOpenFileName(self.centralwidget, "Select File", os.getcwd(),"Mbarete files (*.mbarete)")
	    path_data = []
