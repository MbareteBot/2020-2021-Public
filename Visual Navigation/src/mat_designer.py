...
class matDesignerWidget(Ui_MainWindow):
    ...
    def setPainter(self, color):
        ...
        self.pen = QtGui.QPen(color, 7, QtCore.Qt.SolidLine,
    ...
    def mouseReleaseEvent(self, event):
        ...
        if self.lastPressedBtn == "SQUARE":
            if len(self.squareClicksPositions) > 0:
                self.rects.append([self.squareClicksPositions[-1][0], self.squareClicksPositions[-1][1], self.globalMX -
                                   self.squareClicksPositions[-1][0], self.globalMY - self.squareClicksPositions[-1][1]])
    ...
    def drawRectsAndLines(self):
        ...
        if len(self.rects) > 0:
            ...
            for rect in range(len(self.rects)):
                self.painter.drawRect(
                    self.rects[rect][0], self.rects[rect][1], self.rects[rect][2], self.rects[rect][3])
        ...
        if len(self.clicksPositions) > 1:
            ...
            for click in range(len(self.clicksPositions)-1):
                self.painter.drawLine(self.clicksPositions[click][0], self.clicksPositions[click]
                                      [1], self.clicksPositions[click+1][0], self.clicksPositions[click+1][1])
            self.painter.end()
    ...
    def setButtonsActiveStates(self):
        ...
        if self.lastPressedBtn == "SQUARE":
            ...
            rectRgbColor = "rgb" + \
                str((rectQColor[0], rectQColor[1], rectQColor[2]))
        ...
        if self.lastPressedBtn == "LINE":
            ...
            lineRgbColor = "rgb" + \
                str((lineQColor[0], lineQColor[1], lineQColor[2]))
            self.Btn_colorPick.setStyleSheet("QPushButton {\n"
                                             f"background-color: {lineRgbColor};\n"
                                             "image: url(:/img/img/drop-silhouette.png);}")

    # ANIMATES THE "DRAG AND DROP" ANIMATION WHEN DRAWING A RECT - JUST THAT :)
    ...
    def rectDrawing(self, event):
        ...
        if len(self.squareClicksPositions) > 0:
            self.painter.drawRect(self.squareClicksPositions[-1][0], self.squareClicksPositions[-1][1],
                                  self.globalMX - self.squareClicksPositions[-1][0], self.globalMY - self.squareClicksPositions[-1][1])
    ...
    def lineDrawing(self, event):
        ...
        if len(self.clicksPositions) > 0:
            ...
            self.painter.drawPoint(
                self.clicksPositions[0][0], self.clicksPositions[0][1])
    ...
    def goBack(self):
        print(self.graphicsElements)
        for e in self.graphicsElements:
            print("ELEMENT", e)
            if e == "SQUARE":
                ...
                if len(self.rects) > 0:
                    ...
                    self.graphicsElements.remove(e)
                    break
