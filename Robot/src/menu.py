...
def run_menu()
    ev3 = EV3Brick()
    ev3.screen.clear()
    screen_width = self.ev3.screen.width - 1
    screen_height = self.ev3.screen.height - 1
    labels = labels
    label_container_width = self.screen_width
    label_container_height = self.screen_height/len(self.labels)
    defaulf_font_height = 16
    active_label = 0
...
for label in self.labels:
    ...
    # Underline the label
    self.ev3.screen.draw_line(0, last_container_pos, self.screen_width, last_container_pos)
...
def motor_control(self):
    ...
    while True:
        ...
        if Button.CENTER in pressed_buttons:
            ...
            self.devices.left_action_motor.hold()
            self.devices.right_action_motor.hold()
            self.devices.left_steering_motor.hold()
            self.devices.right_steering_motor.hold()
    ...
    def main(self):
        ...
        while True:
            pressed_buttons = self.ev3.buttons.pressed()
            while Button.UP in self.ev3.buttons.pressed() or Button.DOWN in self.ev3.buttons.pressed():
                pass
